package modifying.game.view.tools;
...
public class MouseGameEffect {
    ...
    public static void setAudioCtrl(AudioCtrl audioCtrl) {
        MouseGameEffect.audioCtrl = audioCtrl;
    }

    private MouseGameEffect(Scene scene, Pane rootPane) {
        // 覆盖层（透明，不干扰交互）
...
// 安全后备：移动结束后如果节点还在（例如淡出被跳过），也移除
    ...
    public static MouseGameEffect attach(Scene scene, Pane rootPane) {
        return new MouseGameEffect(scene, rootPane);
    }
